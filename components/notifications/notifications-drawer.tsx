...
const mockNotifications: Notification[] = [
  {
    ...
    message: "Sua consulta com Dr. Carlos Silva (Cardiologia) foi confirmada para amanhã às 09:00.",
  ...
  },
  {
    ...
    message: "Os resultados do seu exame de sangue estão disponíveis para visualização.",
  ...
  },
  {
    ...
    message: "Lembrete: Você tem uma consulta agendada com Dra. Ana Santos (Dermatologia) amanhã às 14:00.",
  ...
  },
  {
    ...
    message: "Sua consulta com Dr. Miguel Costa (Neurologia) foi cancelada devido à indisponibilidade do médico.",
  ...
  },
  {
    ...
    message: "Sua receita para medicamento contínuo foi renovada e está disponível para download.",
  ...
  },
  {
    ...
    message: "O horário da sua consulta com Dra. Sofia Martins (Pediatria) foi alterado para 10:30.",
...
function NotificationCard({ notification, onMarkAsRead }: { notification: Notification; onMarkAsRead: (id: string) => void }) {
  ...
  return (
    <div 
      className={`relative flex flex-col gap-1 rounded-lg p-3 transition-colors ${getNotificationColor(notification.type)} ${notification.read ? "opacity-70" : ""}`}
      onClick={handleClick}
    >
      <div className="flex items-start justify-between">
        ...
        {!notification.read && (
          <Badge variant="secondary" className="ml-1 h-2 w-2 rounded-full p-0 bg-blue-500" />
        )}
      </div>
      <div className="flex items-center justify-between mt-1">
        <p className="ml-7 text-xs text-muted-foreground">{formatNotificationDate(notification.date)}</p>
        {notification.link && (
          <a 
...
export function NotificationsDrawer({ open, onOpenChange, onNotificationsViewed }: NotificationsDrawerProps) {
        ...
        <ScrollArea className="flex-1 px-4">
          {notifications.length > 0 ? (
            ...
            <div className="flex h-40 flex-col items-center justify-center py-4">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Não há notificações</p>
            </div>
          )}
        </ScrollArea>
